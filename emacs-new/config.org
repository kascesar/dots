...
#+end_src
** ORG [6/7]
- [ ] instalacion de talext y otros para renderizar ecuaciones
...
#+begin_src emacs-lisp
  ...
  "Activar el entorno virtual de Python si existe en la carpeta del proyecto."
  (let ((venv-dir (expand-file-name ".venv" (locate-dominating-file default-directory ".venv")))))
    (when (and (file-directory-p venv-dir)
               (file-exists-p (expand-file-name "bin/activate" venv-dir)))
      (pyvenv-activate venv-dir)))

  (add-hook 'python-mode-hook 'my-python-venv-activate)
  (defun set-flycheck-python-pylint-executable ()
  "Configurar Flycheck para usar pylint del entorno virtual activo."
  (let ((venv-python (expand-file-name "bin/python" (pyvenv-workon)))))
    (when (file-exists-p venv-python)
      (setq-local flycheck-python-pylint-executable venv-python)))

  (add-hook 'python-mode-hook 'set-flycheck-python-pylint-executable)
#+end_src

#+RESULTS:
: my-python-venv-activate
