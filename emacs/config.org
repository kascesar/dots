...
;; Header scaling — independiente para que no dependa del cálculo de color
(defun my/org-scale-headers ()
  (set-face-attribute 'org-level-1 nil :height 2.0 :weight 'bold)
  (set-face-attribute 'org-level-2 nil :height 1.6 :weight 'bold)
  (set-face-attribute 'org-level-3 nil :height 1.3)
  (set-face-attribute 'org-level-4 nil :height 1.1))
(add-hook 'org-mode-hook #'my/org-scale-headers)
(with-eval-after-load 'ef-themes
  (add-hook 'ef-themes-post-load-hook #'my/org-scale-headers))
...
(defun my/org-add-block-overlays ()
  (save-excursion
    (let ((case-fold-search t))
      (goto-char (point-min))
      (while (re-search-forward "^[ \t]*#\\+\\(?:begin\\|end\\|results\\)[_:].*$" nil t)
