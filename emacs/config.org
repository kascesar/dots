...
(defun my/org-set-keyword-at-point ()
  (interactive)
  (let* ((keywords '((?p "PORHACER")
                     (?e "ENPROCESO")
                     (?b "BLOQUEADO")
                     (?d "DETENIDO")
                     (?h "HECHO")
                     (?a "ARCHIVAR")))
         (prompt (mapconcat (lambda (k)
                              (concat "[" (char-to-string (car k)) "] "
                                      (propertize (cadr k) 'face (org-get-todo-face (cadr k)))))
                            keywords "  "))
         (ch (read-char-choice (concat "Estado: " prompt "  ") (mapcar #'car keywords)))
         (keyword (cadr (assq ch keywords)))
         (word (thing-at-point 'word t))
         (in-keyword (cl-member word keywords :test (lambda (w k) (string= w (cadr k))))))
    (if in-keyword
