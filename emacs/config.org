...
;; display "" sobreescribe display "◉" de org-modern — invisible no gana contra display
(defvar my/org-reading-fl-keywords
  '(;; Estrellas de headings: * ** *** → desaparecen con el espacio
    ("^\\(\\*+ \\)"
     (1 '(face nil display "") t))
    ;; Bullets de lista: - item / + item
    ("^[ \t]*\\([-+] \\)"
     (1 '(face nil display "") t))))

(defun my/org-clear-reading-display ()
  "Elimina las text properties display=\"\" que pusimos al activar lectura."
  (with-silent-modifications
    (save-excursion
      (goto-char (point-min))
      (while (re-search-forward "^\\(\\*+ \\)\\|^[ \t]*\\([-+] \\)" nil t)
        (let ((beg (or (match-beginning 1) (match-beginning 2)))
...
(defun my/org-toggle-reading-mode ()
  ...
  (if my/org-reading-mode
      (progn
        ...
        (font-lock-add-keywords nil my/org-reading-fl-keywords 'append)
        (font-lock-flush)
    ...
    (progn
      ...
      (font-lock-remove-keywords nil my/org-reading-fl-keywords)
      (my/org-clear-reading-display)
      (font-lock-flush))))
